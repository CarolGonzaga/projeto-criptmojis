...
let difficulty;
let secretCode = [];
let selectedEmojis = [null, null, null, null, null];
...
function updateBetDisplay() {
    ...
    const betEmojisContainer = document.querySelector(".bet-emojis");
...
function removeEmojiFromBet(index) {
    selectedEmojis[index] = null;
    updateBetDisplay();
}
function customSort(list) {
    return list.sort((a, b) => {
        const order = { match: 0, partial: 1, wrong: 2 };
        return order[a] - order[b];
    });
}
// Função para configurar o botão de envio
function setupSendButton() {
    const sendBtn = document.querySelector(".bet-button");
    let listResult = [];
    if (!sendBtn) {
    ...
    sendBtn.addEventListener("click", () => {
        if (selectedEmojis.some((item) => item === null || item === "")) {
            ...
            const sequenceContainer = document.querySelector(".attempts-content");
...
function updateUI() {
    ...
    const attemptCount = document.querySelector("#numAttempts");
    if (gameDifficulty)
        gameDifficulty.textContent = difficulty;
    if (attemptCount)
        attemptCount.textContent = formatNumAttempts(numAttempts);
}
function updateScore() {
    const scoreCount = document.querySelector("#player-score");
    if (scoreCount)
...
function enableGameControls() {
    ...
    emojiButtons.forEach(btn => btn.disabled = false); // Bloqueia os botões de emoji
}
function disableGameControls() {
    ...
    emojiButtons.forEach(btn => btn.disabled = true); // Bloqueia os botões de emoji
}
function showCode() {
...
function calculateScore(difficulty, attemptsLeft) {
    let baseScore = 0;
    switch (difficulty) {
        case "Tranquilo":
            baseScore = 100;
...
function resetGame() {
    ...
    if (sequenceContainer)
        sequenceContainer.innerHTML = ""; // Apaga todas as apostas já feitas
...
function setupNivelMenu() {
    ...
    document.addEventListener("click", (event) => {
        if (!menuNivel.contains(event.target) && event.target !== btnNivel) {
            menuNivel.classList.remove("show");

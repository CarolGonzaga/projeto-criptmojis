...
export function addEmojiToBet(emoji) {
...
export function clearBetDisplay() {
    const betEmojisContainer = document.querySelector(".bet-emojis");
    if (!betEmojisContainer) {
        console.error("Erro: Container de bet-emojis não encontrado!");
        return;
...
// Inicia o jogo
export function startGame(attempts, selectedDifficulty) {
...
export function formatNumAttempts(numAttempts) {
...
export function updateUI() {
    ...
    if (gameDifficulty)
        gameDifficulty.textContent = gameState.difficulty;
    if (attemptCount)
        attemptCount.textContent = formatNumAttempts(gameState.numAttempts);
}
...
export function endGame() {
    ...
    if (compareLists(gameState.secretCode, gameState.selectedEmojis)) {
...
export function showCode() {
    ...
    tagCodes.forEach((code, index) => {
        code.textContent = `${gameState.secretCode[index]}`;
    });
}
...
export function calculateScore(difficulty, attemptsLeft) {
    let baseScore = 0;
    switch (difficulty) {
        case "Tranquilo":
            baseScore = 100;
